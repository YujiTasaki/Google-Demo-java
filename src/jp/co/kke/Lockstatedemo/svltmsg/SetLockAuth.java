...
public class SetLockAuth extends AbstractMngMessage{
	//@SuppressWarnings("unused")
	private static Logger logger = Logger.getLogger(SetLockAuth.class);
	...
	public void doJob(Map<String, Object> hArg, HttpServletRequest request, HttpServletResponse response)	throws Exception {
		pharseArg(hArg);
		this.accessToken = this.getServlet().getMngLockApi().requestAccessToken(code);
	...
	public void returnOk(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException{
		request.setAttribute(ServletUtil.S_REQ_ATT_KEY_INFO, "Lock認証成功:" + this.accessToken);
